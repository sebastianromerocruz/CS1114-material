...
***Song of the day***: _[**Need You Tonight**](https://youtu.be/F93ywiGMDnQ) by INXS (1987)._
...
The 8am sections had a pop-quiz during lab last Friday—the first of the semester—so congratulations on getting through that! Let's take a closer look at the problem statement:

```
Ask the user two input two numbers (you can assume that they will always enter numerical characters). Then, write a program that will generate a random integer between 0 and 100, inclusive on both ends. Finally, your program should print True if:
...
For those of you who weren't there for the quiz, the solution is fairly straight forward if you are comfortable with _conditional statements_. Asking for input from the user, as well as generating a random number shouldn't be too bad either:
...
Printing `True` or `False` was a matter of remember that _all conditional expressions evaluate to either `True` or `False`_. For example, if we execute the following line:
