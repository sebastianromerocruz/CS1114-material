...
***Song of the day***: _[**Ain't Nobody Know**](https://youtu.be/deIS1BmSfZQ) by Gen Hoshino (2019)._
...
[**file**](video_game.csv) that contains information for a large number of video games:
...
_**Figure 1**: First ten lines of [**video_game.csv**](video_game.csv) 
([**Source**](https://www.kaggle.com/juttugarakesh/video-game-data))._

Write a function called `extract_data()` that accepts the file's address as its only parameter, and returns a list of 
...
Oof. That's quite a lot. But hey, we accomplished our task! The problem really arises when, for example, you show this 
to your boss, and they ask you something like:
...
_**Figure 2**: Python's data type hierarchy...thus far. (Courtesy of Prof. Reeves)_
...
_**Figure 3**: Python's container hierarchy. Now with dictionaries! (Courtesy of Prof. Reeves)_
...
The code is actually almost identical to our earlier function. The key (no pun intended) differences are that instead of
initializing an empty list, we initialize an empty dictionary. The file opening process stays exactly the same:
...
Assuming that our file does open, we will now need to iterate and prepare each line for data extraction. This, too,
...
Now for the new stuff. For each line of this file, I want to create a new dictionary entry; a new key-value pair. As you
...
Don't forget to close your file and return your dictionary! [**Here's**](ExtractData.py) the full program.
